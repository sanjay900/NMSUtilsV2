...
public class AnimatedMessage {
	...
	public AnimatedMessage(File gifFile, int delay, int height, char imgChar) throws IOException {
		this.delay = delay;
	...
	public AnimatedMessage(URL gifFile, int delay, int height, char imgChar) throws IOException {
		this.delay = delay;
		List<BufferedImage> frames = getFrames(gifFile);
		images = new ImageMessage[frames.size()];
		for (int i = 0; i < frames.size(); i++) {
	...
	public List<BufferedImage> getFrames(URL url) {
